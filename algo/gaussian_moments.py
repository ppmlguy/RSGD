...
def compute_lap_moment(eps, lmbd):
    term1 = (lmbd + 1.0)/(2.*lmbd + 1.0)
    term1 *= np.exp(lmbd*eps)
    term2 = lmbd / (2.*lmbd + 1.0)
    term2 *= np.exp(-eps*(lmbd + 1.0))

    moment = np.log(term1 + term2)
...
if __name__ == "__main__":
    ...
    eps, delta = get_privacy_spent(log_moments, target_delta=1e-8)
